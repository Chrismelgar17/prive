...
export function ZonePageClient({ neighborhood, h1, description, seoContent, breadcrumbSchema }: Props) {
  const [profiles, setProfiles] = useState<Therapist[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    getProfiles().then(all => {
      setProfiles(all.filter(t => !t.isPaused && t.neighborhood === neighborhood))
      setLoading(false)
    })
  }, [neighborhood])

  const otherZones = ALL_ZONES.filter(z => !z.href.includes(neighborhood.toLowerCase()))
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 py-8">
        ...
        <nav aria-label="Breadcrumb" className="text-xs text-white/40 mb-6">
          <ol className="flex items-center gap-1.5">
            ...
            <li className="text-white/60">Zonas</li>
            <li className="text-white/20">/</li>
            <li className="text-[#D4AF37]">{neighborhood}</li>
          </ol>
        </nav>
        ...
        {loading ? (
          ...
          <p className="text-white/40 text-center py-12 border border-white/5 rounded-xl">
            No hay perfiles disponibles en {neighborhood} por el momento.{' '}
        ...
        <section className="mt-12 border-t border-white/10 pt-8 grid grid-cols-1 sm:grid-cols-2 gap-8">
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-white/40 mb-4">Otras zonas</h2>
          ...
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-[0.2em] text-white/40 mb-4">Categorías de masajes</h2>
            <ul className="space-y-2.5">
